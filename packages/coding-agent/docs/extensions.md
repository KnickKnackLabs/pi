...
Extensions can transform or replace built-in tools (`read`, `bash`, `powershell`, `edit`, `write`, `grep`, `find`, `ls`). Interactive mode displays a warning when an extension replaces a built-in tool.
