...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://pi.dev/api/report-install` and Pi attribution headers for OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out disables both, but does not disable update checks. Upstream builds fetch `https://pi.dev/api/latest-version`; KKL builds query the latest `KnickKnackLabs/pi` GitHub release and must be updated through the package manager or wrapper that installed them.
...
For Git SemVer ranges, `"update": "startup"` opts the package into a freshness check before resources load. Pi checks at most once every 24 hours, applies a newer compatible tag only to a clean managed checkout, and preserves the existing checkout when offline, dirty, divergent, locked, or when preparation fails. Exact Git refs never move under this policy. If automatic updating refuses a checkout, subsequent resource loading does not repair it: loading stops until the local work is preserved and the checkout is resolved manually. Loading also stops while another updater holds the package lock; a clean compatible checkout remains usable after a failed update check or preparation.
