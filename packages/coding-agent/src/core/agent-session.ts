...
export class AgentSession {
	...
	get isIdle(): boolean {
		return !this._isAgentRunActive && !this.isCompacting && this._pendingExtensionMessageActions === 0;
	}
	...
	private async _drainQueuedExtensionCommands(): Promise<boolean> {
		...
		// agent_end handlers can enqueue context after the last turn_end flush.
		// Persist it on the outgoing branch before a terminal command can navigate.
		this._flushPendingCustomMessages();
		this._persistActiveAgentSegmentCompletion();
