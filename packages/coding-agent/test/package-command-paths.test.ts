	...
	async function runPackageCommandDirectly(args: string[]): Promise<void> {
		expect(
			await handlePackageCommand(args, {
				latestReleaseProvider: (version) => getLatestPiRelease(version, { channel: "upstream", retry: true }),
...
`,
		...
		const kklVersion = "0.84.3-kkl.1";
		await expect(
			handlePackageCommand(["update", "--self"], {
				latestReleaseProvider,
				selfUpdateContext: { packageName: PACKAGE_NAME, version: kklVersion },
			}),
		).resolves.toBe(true);

		expect(process.exitCode).toBe(1);
		expect(existsSync(recordPath)).toBe(false);
		expect(latestReleaseProvider).toHaveBeenCalledWith(kklVersion);
		const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
	...
	it("keeps inherited managed state non-managed and uses the check version for forced self-updates", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
