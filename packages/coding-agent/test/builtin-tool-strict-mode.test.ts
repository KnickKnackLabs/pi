...
import type { AnyToolDefinition } from "../src/core/extensions/types.ts";
...
describe("strict built-in tools", () => {
	...
	it("preserves explicit opt-outs when wrapping definitions for execution", () => {
		...
		for (const name of strictToolNames) {
			...
			const override: AnyToolDefinition = {
				...definition,
				constrainedSampling: false,
			};
			expect(wrapToolDefinitions([override])[0].constrainedSampling).toBe(false);
				...
				extensionFactories: [
					(pi) => {
						pi.on("session_start", () => {
							...
							// Keep each tool's distinct parameter, result, and renderer-state types at registration.
							pi.registerTool({ ...definitions.read, constrainedSampling: false });
							pi.registerTool({ ...definitions.bash, constrainedSampling: false });
							pi.registerTool({ ...definitions.powershell, constrainedSampling: false });
							pi.registerTool({ ...definitions.edit, constrainedSampling: false });
							pi.registerTool({ ...definitions.write, constrainedSampling: false });
