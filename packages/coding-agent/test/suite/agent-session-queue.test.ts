...
describe("AgentSession queue characterization", () => {
	...
	it("runs direct steering and follow-up messages through input handlers", async () => {
		...
		// Input interception and public delivery must agree on the original source.
		expect(
			harness.events.flatMap((event) =>
				event.type === "message_start" && event.message.role === "user"
					? [{ text: getMessageText(event.message), source: event.source }]
					: [],
			),
		).toEqual([
			{ text: "transformed: start", source: "interactive" },
			{ text: "transformed: steer me", source: "rpc" },
			{ text: "transformed: follow me", source: "rpc" },
		]);
	});
